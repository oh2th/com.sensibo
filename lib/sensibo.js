...
module.exports = class Sensibo {
  ...
  async getAllDeviceInfo(apiKey) {
    return http({
      uri: `${this.getUri()}/users/me/pods?fields=id,room,productModel,acState,measurements,motionSensors,timer,connectionStatus&apiKey=${apiKey}`,
      json: true
    });
  }
  ...
  getRemoteCapabilities() {
    return http({
      uri: `${this.getUri()}/pods/${this.getDeviceId()}?fields=measurements,remoteCapabilities&apiKey=${this.getApiKey()}`,
      json: true
    });
  }
  ...
  async getSpecificDeviceInfo() {
    return http({
      uri: `${this.getUri()}/pods/${this.getDeviceId()}?fields=measurements,acState,timer&apiKey=${this.getApiKey()}`,
      json: true
    });
  }
