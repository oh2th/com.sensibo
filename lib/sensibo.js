...
module.exports = class Sensibo {
	...
	async enableClimateReact(enabled) {
		...
		const response = await axios(`${this.getUri()}/pods/${this.getDeviceId()}/smartmode?apiKey=${this.getApiKey()}`, { enabled: enabled });
		return response.data;
	}
	...
	async setCurrentTimer(minutesFromNow, acState) {
		...
		const response = await axios({
			method: 'put',
			url: `${this.getUri()}/pods/${this.getDeviceId()}/timer/?apiKey=${this.getApiKey()}`,
