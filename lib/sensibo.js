...
module.exports = class Sensibo {
  ...
  async isTimerEnabled() {
    ...
    this._logger('isTimerEnabled', this.getDeviceId(), result.timer);
    return result.timer && result.timer.isEnabled === true;
  }

  async deleteCurrentTimer() {
    let data = await http.delete({
      uri: `${this.getUri()}/pods/${this.getDeviceId()}/timer?apiKey=${this.getApiKey()}`,
      json: true
    });
    this._logger('deleteCurrentTimer', this.getDeviceId());
    return data;
  }
