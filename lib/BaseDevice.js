...
module.exports = class BaseDevice extends Homey.Device {
  ...
  async fetchRemoteCapabilities() {
    try {
      ...
      if (data.data) {
        ...
        let result = { ...remoteCapabilities, ...remoteMeasurements };
  ...
  async onRemoteCapabilitiesReceived(remoteCaps) {
    // Add Sensibo device model specific measurement capabilities.
    if (remoteCaps.measurements.tvoc) {
      await this.addCapability('measure_tvoc');
    }
    if (remoteCaps.measurements.co2) {
      await this.addCapability('measure_co2');
    }
    if (remoteCaps.measurements.iaq) {
