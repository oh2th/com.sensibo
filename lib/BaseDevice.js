...
module.exports = class BaseDevice extends Homey.Device {
  ...
  async onRemoteCapabilitiesReceived(remoteCaps) {
    ...
    if (remoteCaps.filtersCleaning.acOnSecondsSinceLastFiltersClean !== undefined
      && remoteCaps.filtersCleaning.filtersCleanSecondsThreshold !== undefined
      && this.hasCapability('se_filter_due_date') === false) {
  ...
  async onDeviceInfoReceived(result) {
    if (result) {
      ...
      if (result.filtersCleaning.shouldCleanFilters !== undefined) {
        await this.updateIfChanged('alarm_filter', result.filtersCleaning.shouldCleanFilters);
        this.homey.app._filterAlarmTrigger.trigger(this, { state: this.getCapabilityValue('alarm_filter') }, {});
      }
      if (typeof result.filtersCleaning.acOnSecondsSinceLastFiltersClean === 'number'
        && typeof result.filtersCleaning.filtersCleanSecondsThreshold === 'number') {
          ...
          if (filterCleanTimeLeft > 0) {
            ...
            await this.updateIfChanged('se_filter_due_date', dueDateFormatted);
          }
        }
    }
  }
